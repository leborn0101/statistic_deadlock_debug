...
void task_5(const void* a){
	...
	while(cnt<LES_EXECTIMES){
		...
		ACoreOs_semaphore_obtain(semaphore_mutex4, ACOREOS_WAIT, ACOREOS_FOREVER);
		DLK_UPTSITE();
		printk_spinLock("in turns of %d task_5 do something!!!\n", cnt);
		ACoreOs_semaphore_release(semaphore_mutex4);
		ACoreOs_semaphore_release(semaphore_mutex3);
		delay(100);
		cnt++;
...
void task_6(const void* a){
	...
	while(cnt<LES_EXECTIMES){
		...
		ACoreOs_semaphore_obtain(semaphore_mutex4, ACOREOS_WAIT, ACOREOS_FOREVER);
		DLK_UPTSITE();
		printk_spinLock("in turns of %d task_6 do something!!!\n", cnt);
		ACoreOs_semaphore_release(semaphore_mutex4);
		ACoreOs_semaphore_release(semaphore_mutex3);
		delay(100);
		cnt++;
...
void task_7(const void* a){
	...
	while(cnt<LES_EXECTIMES){
		...
		ACoreOs_semaphore_obtain(semaphore_mutex3, ACOREOS_WAIT, ACOREOS_FOREVER);
		DLK_UPTSITE();
		printk_spinLock("in turns of %d task_7 do something!!!\n", cnt);
		ACoreOs_semaphore_release(semaphore_mutex4);
		ACoreOs_semaphore_release(semaphore_mutex3);
		delay(100);
		cnt++;
...
void task_8(const void* a){
	...
	while(cnt<LES_EXECTIMES){
		...
		ACoreOs_semaphore_obtain(semaphore_mutex2, ACOREOS_WAIT, ACOREOS_FOREVER);
		DLK_UPTSITE();
		printk_spinLock("in turns of %d task_8 do something!!!\n", cnt);
		ACoreOs_semaphore_release(semaphore_mutex2);
		ACoreOs_semaphore_release(semaphore_mutex1);
		delay(100);
		cnt++;
...
void task_9(const void* a){
	DLK_INIT()
	const char* msg = (const char*)a;
	int cnt = 0;
	while(cnt<LES_EXECTIMES){
		DLK_CMPSITE();
		ACoreOs_semaphore_obtain(semaphore_mutex5, ACOREOS_WAIT, ACOREOS_FOREVER);
		DLK_UPTSITE();
		delay(delay_time);
		DLK_CMPSITE();
		ACoreOs_semaphore_obtain(semaphore_mutex6, ACOREOS_WAIT, ACOREOS_FOREVER);
		DLK_UPTSITE();
		printk_spinLock("in turns of %d task_9 do something!!!\n", cnt);
		ACoreOs_semaphore_release(semaphore_mutex6);
		ACoreOs_semaphore_release(semaphore_mutex5);
		delay(100);
		cnt++;
...
void task_10(const void* a){
	...
	while(cnt<LES_EXECTIMES){
		...
		ACoreOs_semaphore_obtain(semaphore_mutex6, ACOREOS_WAIT, ACOREOS_FOREVER);
		DLK_UPTSITE();
		printk_spinLock("in turns of %d task_10 do something!!!\n", cnt);
		ACoreOs_semaphore_release(semaphore_mutex6);
		ACoreOs_semaphore_release(semaphore_mutex5);
		delay(100);
		cnt++;
